...
export function TopicsTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {pinned.length > 0 && (
        <section>
          <h3 className="mb-2 text-xs font-bold uppercase tracking-wide text-tinte-leise">📌 Angeheftet</h3>
          <div className="grid gap-2.5 lg:grid-cols-2">{pinned.map(card)}</div>
        </section>
      )}
      {tags.map((tg) => (
        <section key={tg || "_"}>
          <h3 className="mb-2 text-xs font-bold uppercase tracking-wide text-tinte-leise">{tg ? committeeLabel(tg) : "Allgemein"}</h3>
  ...
  return (
    <button onClick={onOpen} className="card flex w-full items-center gap-3 p-4 text-left">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 text-xs text-tinte-leise">
...
function CreateFolderSheet({ init, onClose }: { init: { parentId: string | null; tag: string }; onClose: () => void }) {
  ...
  const btn = (active: boolean, label: string, onClick: () => void) => (
    <button
      ...
      className={`rounded-xl border py-2 text-sm font-bold transition ${
        active ? "border-brand bg-brand text-white" : "border-papier-linie dark:border-slate-700"
      }`}
    >
      {label}
    </button>
  ...
  return (
    <Sheet open onClose={onClose}>
      ...
      {tag && <p className="mb-3 text-xs text-tinte-leise">Kategorie: <b>{committeeLabel(tag)}</b> (dein Komitee)</p>}
      {!tag && <div className="mb-3" />}

      <label className="mb-1 block text-sm font-semibold text-tinte-matt">Wer kann das sehen?</label>
      ...
      <p className="mb-3 text-xs text-tinte-leise">
      ...
      {mode === "custom" && showPersons && (
        <div className="mb-3 rounded-2xl border border-papier-linie p-2 dark:border-slate-700">
          <div className="mb-2 flex items-center gap-2">
            <span className="text-sm font-semibold text-tinte-matt">Personen</span>
            <span className="ml-auto text-xs text-tinte-leise">{members.size} gewählt</span>
          </div>
      ...
      {mode === "custom" && showKoms && (
        <div className="mb-3 rounded-2xl border border-papier-linie p-2 dark:border-slate-700">
          <div className="mb-2 flex items-center gap-2">
            <span className="text-sm font-semibold text-tinte-matt">Komitees</span>
            <span className="ml-auto text-xs text-tinte-leise">{koms.size} gewählt</span>
          </div>
            ...
            {COMMITTEES.map((c) => {
              ...
              return (
                <button key={c.slug} onClick={() => setKoms((s) => { const n = new Set(s); on ? n.delete(c.slug) : n.add(c.slug); return n; })}
                  className={`rounded-full border px-3 py-1 text-xs font-bold transition ${on ? "border-brand bg-brand text-white" : "border-papier-linie text-tinte-matt dark:border-slate-700"}`}>
                  {on ? "✓ " : ""}{c.label}
                </button>
              );
            })}
    ...
    <div className="space-y-4">
      ...
      {showMembers && canEditData && (topic.visibility === "personen" || topic.visibility === "custom") && (
        <div className="rounded-2xl border border-papier-linie p-3 dark:border-slate-700">
          <div className="mb-2 text-sm font-semibold text-tinte-matt">Wer darf rein?</div>
          <input className="field mb-2" placeholder="Person suchen…" value={q} onChange={(e) => setQ(e.target.value)} />
          <div className="max-h-44 overflow-y-auto">
            {profiles.filter((p) => !q || normalize(p.username || "").includes(normalize(q))).map((p) => {
              ...
              return (
                <button key={p.user_id} onClick={() => setMembers(topic.id, topic.title, on ? memberIds.filter((x) => x !== p.user_id) : [...memberIds, p.user_id])}
                  className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-sm hover:bg-papier-matt dark:hover:bg-slate-800">
                  <span className={`flex h-5 w-5 items-center justify-center rounded border text-xs text-white ${on ? "border-brand bg-brand" : "border-papier-linie dark:border-slate-600"}`}>{on ? "✓" : ""}</span>
                  {p.username}
                </button>
              );
            })}
      ...
      {(children.length > 0 || canEditData) && (
        <div>
          <div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-tinte-leise">
            📁 Unterordner
          ...
          {children.length > 0 && (
            <div className="grid gap-2 lg:grid-cols-2">
              {children.map((c) => {
                ...
                return (
                  <button key={c.id} onClick={() => onOpen(c.id)} className="flex items-center gap-2 rounded-xl border border-papier-linie px-3 py-2.5 text-left text-sm font-semibold dark:border-slate-700">
      ...
      <div className="space-y-2">
        {stream.length === 0 && pinnedItems.length === 0 && children.length === 0 && (
          <div className="py-8 text-center text-sm text-tinte-leise">Noch nichts hier. Schreib den ersten Beitrag oder leg einen Unterordner an.</div>
        )}
      ...
      {banned && (
        <div className="rounded-2xl border-2 border-red-300 bg-red-500/5 p-4 text-center text-sm dark:border-red-500/40">
          ...
          {bannedUntil && bannedUntil !== PERMANENT_UNTIL && (
            <div className="mt-1 text-xs text-tinte-leise">
          ...
          {bannedUntil === PERMANENT_UNTIL && <div className="mt-1 text-xs text-tinte-leise">Wende dich an das Stufenteam.</div>}
        </div>
      )}
        ...
        <Sheet open onClose={() => setComposerOpen(false)}>
          ...
          <div className="mb-2 flex gap-1.5 rounded-xl bg-papier-matt p-1 dark:bg-slate-800">
            {TYPE_TABS.map(({ t, label }) => (
              <button key={t} onClick={() => setType(t)} className={`flex-1 rounded-lg py-1.5 text-[13px] font-bold transition ${type === t ? "bg-brand text-white" : "text-tinte-matt"}`}>{label}</button>
            ))}
          </div>
...
function ItemRow({
  ...
  return (
    <div className={`rounded-xl border border-papier-linie p-3 dark:border-slate-700 ${i.done ? "opacity-60" : ""}`}>
      <div className="mb-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-tinte-leise">
        <span className="font-semibold text-tinte-matt dark:text-slate-300">{i.author}</span>
      ...
      {i.type === "umfrage" && i.options && (
        <div className="mt-2 space-y-1.5">
          {i.options.map((o) => {
            ...
            return (
              <button key={o.id} onClick={() => onVote(i.id, o.id)} className={`relative w-full overflow-hidden rounded-lg border px-3 py-2 text-left text-sm font-semibold ${picked ? "border-brand" : "border-papier-linie dark:border-slate-700"}`}>
                <span className="absolute inset-y-0 left-0 bg-brand/10 dark:bg-brand/20" style={{ width: `${pct}%` }} />
                <span className="relative flex items-center gap-2">
                  <span className={`flex h-4 w-4 items-center justify-center rounded-full border text-[10px] text-white ${picked ? "border-brand bg-brand" : "border-papier-linie dark:border-slate-600"}`}>{picked ? "✓" : ""}</span>
                  <span className="flex-1">{o.label}</span>
                  <span className="text-xs text-tinte-leise">{c} · {pct}%</span>
                </span>
              </button>
            );
          })}
